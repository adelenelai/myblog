...
Still, it was quite a long process, involving setting up a Bot account on GitHub, deploying an app on Heroku (for the 'bridge'), configuring reCAPTCHA and adding the necessary changes to  our favourite `_config.yml` as well as `staticman.yml` (luckily, relatively minimal needed thanks to the comprehensive theme, but it still required knowing what goes where).
